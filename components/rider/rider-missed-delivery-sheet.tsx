  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 px-0 sm:px-6">
      <div
        ...
        className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-[2rem] bg-[#fbf9f4] shadow-[0_-18px_70px_rgba(14,36,25,0.22)] outline-none"
        tabIndex={-1}
      >
        <div className="sticky top-0 z-10 bg-[#fbf9f4] px-5 pb-4 pt-3 sm:px-8">
          ...
          <div className="mt-5 flex items-start justify-between gap-4">
            <div>
              ...
              <h2 id="rider-missed-title" className="mt-2 font-display text-3xl font-black text-[#012d1d]">
                Missed Delivery
              </h2>
              <p className="mt-2 text-xl leading-8 text-[#414844]">
        ...
        <div className="space-y-6 px-5 pb-6 pt-2 sm:px-8">
          <fieldset>
            <legend className="text-2xl font-black text-[#012d1d]">Reason</legend>
            <p className="mt-1 text-base font-black text-[#414844]">Select one reason</p>
            <div className="mt-4 grid gap-3">
              {missedReasonOptions.map((reason) => (
                <label
                  key={reason}
                  className="flex min-h-16 items-center gap-4 rounded-2xl border border-[#c1c8c2] bg-white px-4 text-lg font-semibold text-[#1b1c19]"
                >
                  <input
                    type="radio"
                    name="missed-reason"
          ...
          <label className="block">
            <span className="text-2xl font-black text-[#012d1d]">Optional Note</span>
            <span className="mt-1 block text-base leading-6 text-[#414844]">Add a short note for admin review only.</span>
            <textarea
              value={note}
              onChange={(event) => setNote(event.target.value)}
              placeholder="Type a short note…"
              className="mt-3 min-h-28 w-full rounded-2xl border border-[#c1c8c2] bg-white px-4 py-3 text-base text-[#1b1c19] outline-none ring-[#012d1d] focus:ring-2"
            />
          </label>
